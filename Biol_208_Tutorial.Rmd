...
numStillAlive <- function(dataframeCol) { 
  totalStartAlive <- sum(dataframeCol) #find the total number of people alive
  for (i in nrow(dataframeCol)){ #for every row (age class)
    sumDead <- rowSums(subset1[1:i,1]) #add every row including that one to find the total dead
...
```{r}
DemographySubset$Pre1840FStillAlive <- numStillAlive(DemographySubset$Pre.1840.Female)
```
